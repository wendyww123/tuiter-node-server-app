import people from './users.js' // import the array of users. Include the extension
let users = people

const UserController = (app) => {   // use express instance app to declare HTTP GET
    app.get('/api/users', findUsers) // request pattern /api/users to call a function
    app.get('/api/users/:uid', findUserById) // request pattern /api/user/123 to call a function
    app.post('/api/users', createUser) // request pattern /api/users to call a function
    app.delete('/api/users/:uid', deleteUser) //map URL pattern to handler function
    app.put('/api/users/:uid', updateUser);

}
const findUsers = (req, res) => { // function runs when /api/users requested
    const type = req.query.type     // get query parameter from URL
    if (type) {                     // if type is not null
        const usersOfType = users.filter(user => user.type === type)
        res.json(usersOfType)        // responds with JSON array of users of type
        return
    }
    res.json(users)                // responds with JSON array of users
}

const findUserById = (req, res) => {
    const userId = req.params.uid;
    const user = users
        .find(u => u._id === userId);
    res.json(user);
}

const createUser = (req, res) => {
    const newUser = req.body;
    newUser._id = (new Date()).getTime() + '';
    users.push(newUser);
    res.json(newUser);
}

const deleteUser = (req, res) => {
    const userId = req.params['uid'];
    users = users.filter(usr =>
        usr._id !== userId);
    res.sendStatus(200);
}

const updateUser = (req, res) => {
    const userId = req.params['uid'];
    const updates = req.body;
    users = users.map((usr) =>
        usr._id === userId ?
            {...usr, ...updates} :
            usr
    );
    res.sendStatus(200);
}

export default UserController   // exports so app.js can import
